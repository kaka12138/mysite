...
def get_blog_list_common_data(request, blogs_all_list):
    ...
    page_num = request.GET.get('page', 1)  # 获取url中的页码参数(Get请求)
    page_of_blogs = paginator.get_page(page_num)  # 获取具体页
    ...
    context['blogs'] = page_of_blogs.object_list  # 具体页中的博客数量
    context['blog_types'] = BlogType.objects.annotate(blog_count=Count('blog'))  # annotate: QuerySet对象的方法
    context['blog_dates'] = blog_dates_dict  # 返回包含datetime对象的列表
    return context
...
def blog_detail(request, blog_pk):
    ...
    response.set_cookie(read_cookie_key, 'true')  # 让浏览器保存信息(k:v),阅读cookie的标记
    return response
